...
export default function BetaUserManagement() {
  ...
  async function loadUsers() {
    try {
      setError('');
      setLoading(true);
      const response = await api.get('/admin/beta-users');
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gray-900 flex items-center justify-center">
        ...
        <span className="text-white">Loading beta users...</span>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-purple-900/20 to-gray-900 p-8">
      <div className="max-w-4xl mx-auto">
        <h1 className="text-3xl font-bold text-white mb-8">Beta User Management</h1>
  ...
  return (
    <div className="flex items-center justify-between p-4 bg-gray-700/30 rounded-lg">
      ...
      <div className="flex items-center space-x-2">
        ...
        <span className={`text-sm ${user.status === 'approved' ? 'text-green-400' : 'text-red-400'}`}>{user.status}</span>
      </div>
    </div>
  );
} 