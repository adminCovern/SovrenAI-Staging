    ...
    def get_metrics(self, component: str, window_seconds: int) -> Dict[str, float]:
        ...
        for metric, measurements in self.metrics[component].items():
            ...
            if recent_measurements:
                result[f'{component}_{metric}_mean'] = float(np.mean(recent_measurements))
                result[f'{component}_{metric}_p95'] = float(np.percentile(recent_measurements, 95))
                result[f'{component}_{metric}_p99'] = float(np.percentile(recent_measurements, 99))
